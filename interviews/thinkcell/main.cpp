...
#ifdef TESTING
using std::vector;
#define TESTVALUERANGE 10
#define MAXTESTVALUE 10 
#define TESTREPS 3000000
#endif
/**
...
class interval_map {
  ...
  void assign(K const& keyBegin, K const& keyEnd, const V& val) {
    ...
    if (((--m_map.upper_bound(keyBegin))->second) != val) {
        m_map[keyBegin] = val;
    }
  }
...
class testIntervalMapImpl {
  ...
  int ints() {
    int ret = 1;
    for (int j = 1; j < TESTVALUERANGE; j++) {
      if(testV[j - 1] != testV[j]) ret++;
    }
    if(testV[TESTVALUERANGE - 1] != begval){
        ret++;
    }
    return ret;
  }
...
void random_tests() {
  ...
  for (int i = 0; i < TESTREPS; i++) {
    ...
    for (int j = 0; j< TESTVALUERANGE; j++) {
      ...
      assert( (testMap.ints() - map.ints() ) <= 1);
    }
#endif
  }
}
